...
class Clickwhale_Pro_Tracking_Code_Edit {
	...
	private static function default_dynamic_values_text(): string {
		$text = __( "See Dynamic conversion values tags below:", CLICKWHALE_PRO_SLUG ) . "<br>";
		$text .= '<em>';
		$text .= "<strong>@@ORDERID@@</strong>: " . __( "return the order id", CLICKWHALE_PRO_SLUG ) . "<br>";
		$text .= "<strong>@@CURRENCY@@</strong>: " . __( "return the order currency", CLICKWHALE_PRO_SLUG ) . "<br>";
		$text .= "<strong>@@FULLNAME@@</strong>: " . __( "return the user's full name", CLICKWHALE_PRO_SLUG ) . "<br>";
		$text .= "<strong>@@EMAIL@@</strong>: " . __( "return the user's email", CLICKWHALE_PRO_SLUG ) . "<br>";
		$text .= "<strong>@@PRODUCTS@@</strong>: " . __( "return the name of the purchased products separated by a comma",
				CLICKWHALE_PRO_SLUG ) . "<br>";
		$text .= "<strong>@@AMOUNT@@</strong>: " . __( "return the amount of the cart payment",
				CLICKWHALE_PRO_SLUG ) . " (<strong>" . __( "tax and shipping excluded",
				CLICKWHALE_PRO_SLUG ) . "</strong>)<br>";
		...
		$text .= "<strong>@@TOTAL@@</strong>: " . __( "return the total amount of the purchase",
				CLICKWHALE_PRO_SLUG ) . " (<strong>" . __( "tax included", CLICKWHALE_PRO_SLUG ) . "</strong>)";
		$text .= '</em>';
	...
	public function tracking_code_conversion_fields( $item ) {
		...
		if ( $is_woo ) {
			echo Helper::render_control(
				array(
					'row_label'   => __( 'WooCommerce Products for tracking', CLICKWHALE_PRO_SLUG ),
					'control'     => 'select',
		...
		if ( $is_edd ) {
			echo Helper::render_control(
				array(
					'row_label'   => __( 'Easy Digital Downloads Products for tracking', CLICKWHALE_PRO_SLUG ),
					'control'     => 'select',
