...
namespace Clickwhale\Vendor\WhichBrowser\Analyser\Header\Useragent;

use Clickwhale\Vendor\WhichBrowser\Constants;
use Clickwhale\Vendor\WhichBrowser\Data;
use Clickwhale\Vendor\WhichBrowser\Model\Family;
use Clickwhale\Vendor\WhichBrowser\Model\Using;
...
trait Application
{
    ...
    private function detectSpecificApplications($ua)
    {
        ...
        if (preg_match('/Android [0-9\.]+; (.*) Sony\/.*SonySelectSDK\/([0-9\.]+)/iu', $ua, $match)) {
            ...
            $this->data->browser->type = Constants\BrowserType::APP;
            $this->data->browser->using = new \Clickwhale\Vendor\WhichBrowser\Model\Using([
