...
class Clickwhale_Categories_List_Table extends WP_List_Table {
	...
	function column_title( $item ) {
		...
		$actions = array(
			...
			'delete' => sprintf( '<a href="?page=%s&action=delete&id=%d">%s</a>',
				sanitize_text_field( $_REQUEST['page'] ), intval( $item['id'] ), __( 'Delete', 'clickwhale' ) ),
		);
	...
	function prepare_items() {
		...
		if ( isset( $_GET['page'] ) && isset( $_GET['s'] ) ) {
			...
			$this->items = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}clickwhale_categories ORDER BY $orderby $order LIMIT %d OFFSET %d",
